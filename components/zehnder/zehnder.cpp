    ...
    case StateDiscoveryWaitForLinkRequest:
      ...
      switch (pResponse->command) {
        case FAN_NETWORK_JOIN_OPEN:  // Received linking request from main unit
          ...
          // Send response frame
          ESP_LOGD(TAG, "Response Frame: 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%08X",
          pTxFrame->rx_type, pTxFrame->rx_id, pTxFrame->tx_type, pTxFrame->tx_id,
          pTxFrame->ttl, pTxFrame->command, pTxFrame->payload.networkJoinRequest.networkId);
